...
\begin{definition}
  ...
  Let $\b{G}$ be a decorated block graph, let $\spine$ be a spine on $\b{G}$, and let~$\gamma$ be an arc of~$\spine$ with endpoints~$X$ and~$Y$.
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item each arc~$\gamma$ by the intersection~$\labeling(\gamma)$ of the connected component of~$\smash{M \ssm \Up[{\so[\gamma]}]}$ containing~$\ta[\gamma]$ with the connected component of~$\smash{M \ssm \Down[{\ta[\gamma]}]}$ containing~$\so[\gamma]$.
  \end{itemize}
...
%    \item labeling each arc~$\gamma$ of~$\spine$ by the intersection of the connected component of~$\smash{M \ssm \Up[{\so[\gamma]}]}$ containing~$\ta[\gamma]$ with the connected component of~$\smash{M \ssm \Down[{\ta[\gamma]}]}$ containing~$\so[\gamma]$,
...
\begin{proposition}
  ...
  Let~$\b{M} \eqdef (M, \decoration)$ be a decorated maple tree, let~$\b{G} \eqdef (G, \decoration)$ be the decorated block graph obtained by tapping~$M$, and let~$\spine$ be a spine on~$\b{G}$.
  For any cut~$\Gamma$ of~$\spine\blossom$, the labels of the nodes, arcs and blossoms of~$\Gamma$ are precisely the connected components of~$\smash{M \ssm \big( \Up[{\so[\Gamma]}] \cup \Down[{\ta[\Gamma]}] \big)}$.
\end{proposition}
...
\begin{definition}
  ...
  \end{itemize}
%  We denote by~$\labeling$ the label map on nodes and arcs of~$\spine$.
\vincent{Maybe need to say that this is well-defined. Meaning that $X$ is in a connected component of~$M \ssm \big( \Up[{\so[X]}] \cup \Down[{\ta[X]}] \big)$, and same for $\gamma$. Also that the labels are non-empty, because $\labeling(X) \supseteq X$ and because~$\labeling(\gamma)$ contains all vertices of~$M$ neighbors to both~$\so[\gamma]$ and~$\ta[\gamma]$.}
\end{definition}

We will see in \cref{subsec:blossomingSpines} that this labeling behaves very nicely with respect to cuts of~$\spine$.
...
\begin{remark}
  ...
  \begin{enumerate}[(i)]
    \item Since the vertex set of~$G$ is the disjoint union of the nodes of~$\spine$, it is still the disjoint union of the nodes of~$\spine'$.
    \item For an arc~$\gamma$ of~$\spine$ and a connected component~$C$ of~$M \ssm \big( \Up[{\so[\gamma]}] \cup \Down[{\ta[\gamma]}] \big)$ there is a unique connected component~$D$ of ${M \ssm \big( \Up[{\so[{\so[\gamma]}]}] \cup \Down[{\ta[{\so[\gamma]}]}] \big)}$ containing~$C$, and a unique connected component~$E$ of ${M \ssm \big( \Up[{\so[{\ta[\gamma]}]}] \cup \Down[{\ta[{\ta[\gamma]}]}] \big)}$ containing~$C$. Indeed... todo.
