...
\]
whenever $c_i=d_j'$, and units $id_x \in \dioperad(x;x)$ which satisfy appropriate unit, equivariance and associativity axioms, see \cite[Def.~11.18]{YauJohnson15}. 
\end{definition}
...
Let $C$ be the set of all maple trees. 
...
\]
whose basis is made of pairs $(\Maple,B)$ with $\Maple$ a maple tree and $B$ a subset of blue vertices such that the connected components of $\Maple \ssm \Down[B]$ are $x_1,\ldots,x_k$, and the connected components of $\Maple \ssm \Up[B]$ are $y_1,\ldots,y_l$.

The symmetric groups $\mathbb{S}_k$ and $\mathbb{S}_l$ act trivially on the pairs $(\Maple,B)$, making $\dioperad_{\decoratedMaple}$ into a $C$-colored $(\mathbb{S},\mathbb{S})$-module. 
