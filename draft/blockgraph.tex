...
\newcommandx{\Up}[2][1=v, 2=\decoration]{\overline #2(#1)} 
\newcommandx{\Down}[2][1=v, 2=\decoration]{\underline #2(#1)} 
...
\newcommandx{\spinePoset}[1][1=G]{\mathcal{P}_{#1}} % spine poset
% nested complex
...
\newcommandx{\nestedComplex}[1][1=G]{\mathcal{N}_{#1}} % nested complex
% maps
...
\newcommand{\spineToSpine}{\mathsf{S}} % partition to spine
\newcommandx{\surjectionSpines}[2][1=G, 2=G']{\Phi_{#1}^{#2}} % spine to spine
...
\newcommandx{\spineFan}[1][1=G]{\mathcal{S}_{#1}} % spine fan
% polytopes
...
\newcommandx{\Spin}[2][1=G,2=\weight]{\polytope{Spin}^{#2}_{#1}} % spine polytope
...
\begin{example}
  ...
  \begin{enumerate}[(i)]
    \item the \defn{ordered partitions} of~$\vertexSet$ when $G$ is complete or undecorated.
    \item the \defn{$G$-trees} studied in~\cite{Postnikov} when~$G$ is down decorated (in particular the classical \defn{Schr\"oder trees} when~$G$ is a down decorated path).
    \item the \defn{$G$-ordered partitions}~\cite{Pilaud-acyclicReorientationLattices} (\ie pairs~$(p,o)$ where~$p$ is a partition of~$\vertexSet$ into connected subgraphs of~$G$, and~$o$ is an acyclic orientation of the quotient graph~$G/p$) when~$G$ is fully decorated.
...
\begin{definition}
  ...
  Let $G$ be a decorated block graph, let $\spine$ be a spine on $G$, and let~$\gamma$ be an arc of~$\spine$ with endpoints~$X$ and~$Y$.
...
\begin{definition}
  \label{def:block}
  A \defn{block} of~$G$ is a subset~$B$ of vertices of~$G$ such that~$B$ and its complement~$\vertexSet \ssm B$ forms a splittable partition of the vertex set~$\vertexSet$ (see \cref{def:splittable}).
%  A \defn{block} of~$G$ is a subset~$B$ of~$\vertexSet$ such that $B \ne \varnothing$ is contained in a connected component of~$G \ssm \Down[\compl{B}]$ and $\compl{B} \ne \varnothing$ is contained in a connected component of~$G \ssm \Up[B]$.
...
%  A \defn{block} of~$G$ is a subset~$B$ of vertices of~$G$ such that~$B$ and its complement~$\vertexSet \ssm B$ forms a splittable partition of the vertex set of~$G$ (see \cref{def:splittable}).
...
%  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item $B \posDisjoint B'$ if $B \cup B' = \vertexSet$ and~$B \cap B'$ is not a block of~$G$.
  \end{itemize}
...
\begin{definition}
  \label{def:flipGraph}
  The \defn{flip graph} is the graph whose vertices are the maximal spines on~$G$ and whose egdes are the flips between them.
...
\begin{definition}
  ...
  The flip from~$\spine$ to~$\spine'$ in \cref{def:flip} is \defn{$\prec$-incresing} if~$u \prec v$ and \defn{$\prec$-decreasing} if $u \succ v$.
  The \defn{$\prec$-increasing flip graph} is the directed graph whose vertices are the maximal spines on~$G$ and whose arcs are the increasing flips between them.
...
\begin{example}
  ...
  \begin{enumerate}[(i)]
    \item the \defn{weak order} on permutations of~$\vertexSet$ when $G$ is complete or undecorated.
    \item the \defn{flip poset on maximal tubings} of~$G$ studied in~\cite{BarnardMcConville} when~$G$ is down decorated (in particular the classical \defn{Tamari lattice}~\cite{Tamari, HuangTamari} when~$G$ is a down decorated path).
    \item the \defn{acyclic reorientation lattice} of~$G$~\cite{Pilaud-acyclicReorientationLattices} when~$G$ is fully decorated (in particular the \defn{boolean lattice} when~$G$ is a fully decorated tree).
...
\vincent{I want to do that directly from spines of~$G$ to spines of~$G'$ when~$G$ refines~$G'$.}
...
\begin{remark}
  Note that if an ordered partition~$\pi$ of~$\vertexSet$ has~$k$ parts, then the spine~$\partitionToSpine(\pi)$ on~$G$ has at least $k$ nodes (it can have more, see \cref{fig:blossomingSpine}). In particular, the permutations of~$\vertexSet$ are sent to the corank~$0$ spines on~$G$.
\end{remark}
...
We now consider the equivalence relation~$\equiv_G$ whose classes are the fibers of the map~$\partitionToSpine$.

\begin{definition}
  \label{def:equivalenceRelation}
  Let~$\equiv_G$ denote the equivalence relation on ordered partitions of~$\vertexSet$ defined by~$\pi \equiv_G \pi'$ if and only if $\partitionToSpine(\pi) = \partitionToSpine(\pi')$.
\end{definition}

\begin{example}
  \label{exm:equivalenceRelations}
  For instance, the equivalence relation~$\equiv_G$ on permutations is
  \begin{enumerate}[(i)]
    \item the \defn{trivial relation} when $G$ is complete or undecorated.
...
\begin{proposition}
  ...
  The equivalence relation~$\equiv_G$ on ordered partitions of~$\vertexSet$ is the transitive closure of the elementary relations
  \[
    \pi_1 | \dots | \pi_k | \pi_{k+1} | \dots | \pi_p \equiv_G \pi_1 | \dots | \pi_k \cup \pi_{k+1} | \dots | \pi_p
  \]
...
\begin{remark}
  Specializing to permutations, the equivalence relation~$\equiv_G$ on permutations of~$\vertexSet$ is the transitive closure of the elementary relations
  \[
    \sigma_1 \dots \sigma_k \sigma_{k+1} \dots \sigma_{|\vertexSet|} \equiv_G \sigma_1 \dots \sigma_{k+1} \sigma_k \dots \sigma_{|\vertexSet|}
  \]
...
\begin{definition}
  ...
  %Let~$G$ and~$G'$ be two decorated block graphs.
  We say that~$G$ \defn{refines}~$G'$ (and that $G'$ \defn{coarsens} $G$) if they are decorations of the same underlying block graph and $\Down[\vertexSet] \subseteq \Down[\vertexSet']$ while $\Up[\vertexSet] \subseteq \Up[\vertexSet']$.
\end{definition}
...
\begin{proposition}
  ...
  If~$G$ refines~$G'$, then
  \begin{itemize}
    \item any block of~$G'$ is a block of~$G$,
    \item the nested complex~$\nestedComplex[G']$ contains the subcomplex of~$\nestedComplex[G]$ induced by the blocks of~$G'$.
  \end{itemize}
\end{proposition}
...
\begin{proposition}
  ...
  If~$G$ refines~$G'$, then~$\equiv_G$ refines~$\equiv_{G'}$.
\end{proposition}
...
For any ordered partitions~$\pi$ of~$\vertexSet$ and~$\pi'$ of~$\vertexSet'$ corresponding to the same ordered partition of the underlying block graph of~$G$ and~$G'$, \cref{prop:refinementEquivalenceRelations} affirms that the spine~$\spine = \partitionToSpine(\pi')$ of~$G'$ only depends on the spine~$\spine' = \partitionToSpine(\pi)$ of~$G$.
This yields a natural surjection~$\surjectionSpines$ from spines on~$G$ to spines on~$G'$, which is clearly a poset morphism from~$\spinePoset[G]$ to~$\spinePoset[G']$.
...
%\cref{prop:refinementEquivalenceRelations} yields a natural surjection~$\surjectionSpines$ from spines on~$G$ to spines on~$G'$.
%Namely, for a spine~$\spine$ on~$G$, pick any ordered partition~$\pi$ of~$\vertexSet$ such that~$\spine = \partitionToSpine(\pi)$, let~$\pi'$ be the corresponding ordered partition of~$\vertexSet'$ where only the decoration differ, and let~$\spine' = \partitionToSpine(\pi')$.
%This surjection is clearly a poset morphism from~$\spinePoset[G]$ to~$\spinePoset[G']$.
...
\begin{definition}
  \label{def:surjection}
  Let~$M$ be a maple tree with red vertices~$R$ and blue vertices~$B$.
  For a spine~$\spine$ on~$G$, we construct a blossoming spine~$\spineToSpine(\spine)$ on~$M$ in four steps:
  \begin{itemize}[wide, labelwidth=!, labelindent=5pt]
    \item \textbf{construct}
    \begin{itemize}[wide, labelwidth=!, labelindent=10pt]
      \item an arc~$\varepsilon_\gamma^D$ for each arc~$\gamma$ of~$\spine$ and each component~$D$ of~$M \ssm (\Up[{\so[\gamma]}] \cup \Down[{\ta[\gamma]}])$,
      \item a node~$n_X^C$ for each node~$X$ of~$\spine$ and each component~$C$ of~$M \ssm ()$,
    \end{itemize}
    \item \textbf{attach} the arcs~$\alpha^{i-1}_C$ (resp.~$\alpha^i_C$) with~$C \subseteq D$ as incoming (resp.~outgoing)~arcs of the node~$n^i_D$,
    \item \textbf{relabel} the arc~$\alpha^i_C$ with~$C \cap R$ and the node~$\alpha^i_D$ with~$D \cap \pi_i$,
    \item \textbf{contract} into a single arc any path of arcs passing through nodes with empty~labels.
...
\begin{proposition}
  \label{prop:links}
  The link of any face of the nested complex~$\nestedComplex$ of a decorated block graph~$G$ is a join of nested complexes of decorated block graphs.
\end{proposition}
...
%    \item the intersection of the hyperplane~$\Hyp$ with the halfspaces~$\HS_B$ for all blocks of~$G$.
  \end{enumerate}
\end{theorem}
...
\begin{example}
  ...
  \begin{enumerate}[(i)]
    \item the classical \defn{permutahedron}~$\Perm$ when~$G$ is complete or undecorated.
    \item the \defn{graph associahedron} of~$G$ studied in~\cite{CarrDevadoss, Devadoss, Postnikov, FeichtnerSturmfels, Zelevinsky, Pilaud-removahedra} when~$G$ is down decorated (in particular the classical \defn{associahedron} of~\cite{ShniderSternberg, Loday} when~$G$ is a down decorated~path).
    \item the \defn{graphical zonotope}~$\Zono$ when~$G$ is fully decorated (in particular a \defn{parallelepiped} when~$G$ is a fully decorated tree).
...
\begin{proposition}
  ...
  If~$G$ refines~$G'$, then all facet inequalities of~$\Spin[G]$ are facet inequalities of~$\Spin[G']$.
