...
\begin{definition}
  ...
  \vincent{This should not be called block...}
\end{definition}
...
\end{enumerate}
The blue vertex set $V$ inherits a natural numbering by $1,\ldots,|V|$, obtained by starting at the root and going around $M$ in the clockwise direction, numbering a vertex the first time we see it. 
We will consider only blossoming spines on $M$, and refer to them as spines. 

\begin{definition}[Block]
  Let $M$ be a maple tree, with blue vertex set $V$. 
  A \emph{block} $B$ of $M$ is a (possibly empty) node of a spine $S$ on $M$ which has been cut off from $M$. \guillaume{dessin} 
\end{definition}
...
\mathbb{K}\left\{
\begin{array}{cl}
\text{Block } B \text{ of } M \text{ with two bijections }  \\
...
Let $S$ (resp. $S'$) be a spine on $M$ which contains $B$ (resp. $B'$), and let $\gamma$ (resp. $\gamma'$) be the incoming arc such that $(\so[\gamma],\ta[\gamma])=x_i=y_j'=(\so[\gamma'],\ta[\gamma'])$. 
...
\label{prop:dioperadstructure}
  For any maple tree $M$, the preceding partial compositions and units endow the $(\mathbb{S},\mathbb{S})$-module $\mathcal{D}_M$ with a $C$-colored dioperad structure.
\end{proposition}
...
Let $M$ be a maple tree, and let $X \subset V$ be a subset of blue vertices. 
We denote by $M_{/X}$ the extended maple tree obtained by removing the vertices of $X$ and contracting all the adjacent edges and red vertices to a single red vertex, decorated by both the removed vertices of $X$ and the labels of the adjacent red vertices. 
\guillaume{dessin}
\end{definition}

A non-trivial block $B$ of a maple tree $M$ can now be interpreted as a trivial spine on the extended maple tree $M_{/{B^c}}$, where $B^c\coloneqq V\setminus B$ is the complement of $B$ in $V$. 

\begin{definition}[Admissible maple tree]
  Let $M$ be an extended maple tree. 
  An extended maple tree $M'$ is said to be \emph{$M$-admissible} if it can be obtained as the contraction subtree $M_{/{B^c}}$ for some block $B$ on $M$. 
\end{definition}
...
\begin{lemma}
Let $M$ and $M'$ be two extended maple trees, and suppose that there is an $M'$-admissible extended maple tree $\iota(M)$ which is isomorphic to $M$, has the same blue vertices, and such that the label of each of its red vertex contains the label of the corresponding red vertex of $M'$ as a subword. 
...
\begin{remark}
  One could also also consider the totalization \[ \mathcal{D} \coloneqq \bigoplus_M \mathcal{D}_M \] which is a dioperad colored by pairs $(M,c)$ made up of a maple tree and a color of $\mathfrak{C}$. 
\end{remark}

Let $M$ be a maple tree, with blue vertex set $V$. 
A \emph{composite block} $B$ of $M$ is a (possibly empty) subtree of a spine $S$ on $M$ which has been cut off \guillaume{dessin}. 
...
\mathbb{K}\left\{
\begin{array}{cl}
\text{Generalized cell } D \text{ of } M \text{ with two bijections }  \\
...
\begin{proposition} 
  For any maple tree $M$, the preceding partial compositions and units endow the $\mathcal{D}^{\infty}_M$ with a $C$-colored dioperad structure.
\end{proposition}
...
A generalized cell $D$ of $M$ can be interpreted as a spine on the maple tree obtained by contracting the complement of $D$ in $M$, where [...]
Any dioperad $\mathcal{D}^{\infty}_M$ becomes a $\mathfrak{C}$-colored dioperad \guillaume{by setting many spaces to zero}, and the totalization \[ \mathcal{D}_{\infty} \coloneqq \bigoplus_M \mathcal{D}^{\infty}_M \] becomes a dioperad colored by pairs $(M,c)$ made up of a maple tree and a color of $\mathfrak{C}$. 
