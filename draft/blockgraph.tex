...
\begin{proof}
  ...
  We thus just need to check that (i) implies (ii).
    \vincent{todo}
%  Assume thus that~$\spine$ extends~$\spine'$.
%  Since~${\preccurlyeq_{\spine}} \supseteq {\preccurlyeq_{\spine'}}$, we have~${\equiv_{\spine}} \supseteq {\equiv_{\spine'}}$, so that the nodes of~$\spine$ are disjoint unions of the nodes of~$\spine'$.
%  We now distinguish two cases:
%  \begin{itemize}
%    \item Assume first that the nodes of~$\spine$ are precisely the nodes of~$\spine'$. We conclude that~$\spine = \spine'$.
%    \vincent{todo}
%    \item Assume now that there is a node~$X$ of~$\spine$ and at least two nodes~$X'_1, X'_2$ of~$\spine'$ such that~$X \supseteq X'_1 \sqcup X'_2$.
%    \qedhere
%  \end{itemize}
\end{proof}
