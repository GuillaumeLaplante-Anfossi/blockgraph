...
Let $(X,\sim)$ be the set of ordered triples $(M,B,C)$ where $M$ is a decorated maple tree, $B$ is a subset of blue vertices of $M$ and~$C$ is a connected component of $\Maple \ssm \Up[B]$ or $\Maple \ssm \Down[B]$, endowed with the following symmetric relation~$\sim$.
Given two elements~$(M_1,B_1,C_1)$ and~$(M_2,B_2,C_2)$, we say that they are \defn{compatible} and write $(M_1,B_1,C_1)\sim (M_2,B_2,C_2)$ if we have~$M_1=M_2$, $B_1 \cap B_2 = \emptyset$, $B_1 \subset C_2$ and~$B_2 \subset C_1$.
...
\]
is zero, unless we have $M_i=M_j=N_i=N_j \defeq M$, $B_i=B_j=D_i=D_j \defeq B$ for all $i,j$, and there is further
\begin{itemize}
...
such that
\begin{itemize}
  \item $\sigma(i)=C_i$ for each $i$, and
  \item $\tau(j)=D_j$ for each $j$.
\end{itemize}
