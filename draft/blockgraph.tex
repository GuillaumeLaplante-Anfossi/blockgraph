...
\newcommandx{\surjectionSpines}[2][1=\b{G}, 2=\b{G}']{\Phi_{#1}^{#2}} % spine to spine
...
We denote by~$\labeling$ the label map on a blossoming spine.
Note that a maple subtree of~$M$ is completely determined by its leaves, hence by its red vertices.
We thus simplify all pictures of blossoming spines by writing only the red (letters) vertices of the labels.
...
\begin{proposition}
  ...
  Let~$\b{M} \eqdef (M, \decoration)$ be a decorated maple tree and~$\b{G} \eqdef (G, \decoration)$ be the decorated block graph obtained by tapping~$M$.
  For any cut~$\Gamma$ of~$\spine$, the labels of the nodes, arcs and blossoms of~$\Gamma$ are precisely the connected components of~$\smash{M \ssm \big( \Up[{\so[\Gamma]}] \cup \Down[{\ta[\Gamma]}] \big)}$.
\end{proposition}
...
We now want to show that, when~$\b{G}$ refines~$\b{G}'$, there is a natural surjection from the spines on~$\b{G}$ to the spines on~$\b{G}'$.
Here, it is much more convenient to work with maple trees as we need to manipulate both the vertices and the cliques of~$G$.
...
\begin{definition}
  ...
  For a (blossoming) spine~$\spine$ on~$\b{G}$, we construct a (blossoming) spine~$\spineToSpine(\spine)$ on~$\b{G}'$ in three steps:
...
%    \item \textbf{attach} the arcs~$\varepsilon_\gamma^D$ with~$C \subseteq D$ as incoming (resp.~outgoing)~arcs of the node~$n_{\ta[\gamma]}^C$ (resp.~$n^{\so[\gamma]}_D$), for each arc~$\gamma$ of~$\spine$, 
    \item \textbf{attach} the arc~$\varepsilon_\gamma^D$ a an incoming (resp.~outgoing)~arc of the node~$n_X^C$, when~$C \subseteq D$ and~$X = \ta[\gamma]$ (resp.~$X = \so[\gamma]$),
