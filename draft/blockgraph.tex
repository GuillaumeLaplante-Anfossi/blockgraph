...
\begin{definition}
  ...
  We say that~$G$ \defn{refines}~$G'$ (and that $G'$ \defn{coarsens} $G$) if they are decorations of the same underlying block graph and $\down[V_G] \supseteq \down[V_{G'}]$ while $\up[V_G] \supseteq \up[V_{G'}]$.
\end{definition}
...
\begin{proof}[Proof of \cref{thm:permutreehedra}]
  ...
  It follows that the spine polytope of~$G$ realizes the nested complex~$\nestedComplex$ and that its normal fan is the spine fan~$\spineFan$.
\end{proof}

\begin{example}
  \label{exm:permutahedronGraphicalZonotope}
  The spine polytope recovers the permutahedron of \cref{exm:permutahedron} and the graphical zonotope of \cref{exm:graphicalZonotope} in the following two extreme situations:
  \begin{itemize}
