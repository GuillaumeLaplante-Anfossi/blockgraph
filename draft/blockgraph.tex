...
\begin{remark}
  ...
  Namely, we just need to complete the directed tree~$\almostSpine_\bullet$ into the blossoming directed tree~$\almostSpine_\bullet^\blossom$ by adding:
  \begin{itemize}
    \item an incoming (resp.~outgoing) blossom~$\arcSurj{\gamma}{C}$ attached to the unique node~$\nodeSurj{X}{D}$ with~$S \subseteq D$ for each incoming (resp.~outgoing) blossom~$\gamma$ of~$\spine_\circ^\blossom$ attached to node~$X$.
  \end{itemize}
\end{remark}
...
\begin{proof}
  ...
  \begin{enumerate}[(i)]
    ...
    Since~$x$ and~$y$ belong to the source sets of two distinct incoming arcs of~$Z'$, there is~$z \in \Down[Z']$ which separates~$x$ and~$y$ in~$\Maple$.
    Denote by~$Z$ the node of~$\spine_\bullet$ containing~$z$.
    Since~$x \preccurlyeq_{\spine_\bullet'} z$ and~$x$ is closer to~$z$ than to~$y$ in~$\Maple$, our choice of~$\{x,y\}$ ensures that either~$x \preccurlyeq_{\spine_\bullet} z$ or~$z \preccurlyeq_{\spine_\bullet} x$.
    ...
    Otherwise, since~$z \preccurlyeq_{\spine_\bullet} y$ and~$y$ is closer to~$z$ than to~$x$ in~$\Maple$, our choice of~$\{x,y\}$ ensures that either~$z \preccurlyeq_{\spine_\bullet'} y$ or~$y \preccurlyeq_{\spine_\bullet'} z$.
    If~$y \preccurlyeq_{\spine_\bullet'} z$, we obtain that~$y \preccurlyeq_{\spine_\circ} z \preccurlyeq_{\spine_\circ} y$ (since~$\spine_\circ$ extends both~$\spine_\bullet$ and~$\spine_\bullet'$), which contradicts the minimality of~$\spine_\bullet$ (the spine obtained by contracting the path joining~$Z$ to~$Y$ in~$\spine_\bullet$ would still be extended by~$\spine_\circ$).
    We thus obtain that~$x \preccurlyeq_{\spine_\bullet'} z \preccurlyeq_{\spine_\bullet'} y$, contradicting our assumption that~$x \not\preccurlyeq_{\spine_\bullet'} y$.
    \qedhere
  \end{enumerate}
\end{proof}
\vincent{I am here changing $\circ$ $\bullet$ everywhere...}
...
\begin{proposition}
  ...
  If~$\decoratedGraph$ refines~$\decoratedGraph_\bullet$, then the map~$\surjectionSpines$ is 
  \begin{itemize}
    \item a surjection from the spines on~$\decoratedGraph$ to the spines on~$\decoratedGraph_\bullet$,
    \item a filtered map from the spine poset~$\spinePoset[\decoratedGraph]$ to the spine poset~$\spinePoset[\decoratedGraph_\bullet]$, meaning that the rank of~$\surjectionSpines(\spine)$ is at least the rank of~$\spine$ (it can have more, see \cref{fig:blossomingSpine}).
    \vincent{In particular, the image of a maximal spine is a maximal spine.}
  \end{itemize}
\end{proposition}
...
\begin{definition}
  ...
  If~$\decoratedGraph_\circ$ refines~$\decoratedGraph_\bullet$, then the fibers of the surjection~$\surjectionSpines$ define an equivalence relation~$\equivalenceSpines$ on spines of~$\decoratedGraph_\circ$. In other words, $\spine_\circ \equivalenceSpines \spine_\circ'$ if and only if~$\surjectionSpines(\spine_\circ) = \surjectionSpines(\spine_\circ')$.
\end{definition}
...
\begin{proposition}
  ...
  If~$\decoratedGraph$ refines~$\decoratedGraph_\bullet$, then
  \begin{itemize}
    \item any chunk of~$\decoratedGraph_\bullet$ is a chunk of~$\decoratedGraph$,
    \item the chunk nested complex~$\nestedComplex[\decoratedGraph_\bullet]$ contains the subcomplex of~$\nestedComplex[\decoratedGraph]$ induced by the chunks of~$\decoratedGraph_\bullet$.
  \end{itemize}
\end{proposition}
