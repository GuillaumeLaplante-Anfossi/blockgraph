...
However, proving this property requires the following explicit combinatorial description of~$\surjectionSpines(\spine)$, which is illustrated in \cref{fig:refinementSpines}.
...
Our next four statements aim at showing the correctness of \cref{def:refinementSpines}, meaning that~$\spine'$ is indeed a spine on~$\decoratedGraph'$.
...
\begin{lemma}
  ...
  The graph~$\almostSpine'$ is indeed a directed tree.
\end{lemma}

\begin{proof}
  We first prove that~$\almostSpine'$ is connected.
  ...
  For any node~$X$ (resp.~arc~$\gamma$) in the interior of this path, $u$ and~$v$ also belong to the same connected component~$D_\star$~of~$\Maple \ssm \big( \Up[{\so[X]}][\decoration'] \cup \Down[{\ta[X]}][\decoration'] \big)$ (resp.~$C_\star$~of~$\Maple \ssm \big( \Up[{\so[\gamma]}][\decoration'] \cup \Down[{\ta[\gamma]}][\decoration'] \big)$).
  The corresponding nodes~$\nodeSurj{X}{D_\star}$ and arcs~$\arcSurj{\gamma}{C_\star}$ along the path~$\pi$ thus form a path in~$\almostSpine'$ between the node containing~$u$ and the node containing~$v$.
  The connectedness of~$\Graph$ thus implies the connectedness of~$\almostSpine'$.
  
  We now prove that~$\almostSpine'$ is acyclic.
  Assume that~$\almostSpine'$ contains a (non-directed) cycle~$\sigma'$.
  By \cref{lem:refinementSpines1}, the corresponding arcs in~$\spine$ also form a (non-directed, with possible edge repetitions) cycle~$\sigma$.
  Since~$\spine$ is a directed tree, $\sigma$ has two incoming arcs at the same node. Hence, $\sigma'$ also has two incoming arcs at the same node.
  Therefore, there are two paths in~$\almostSpine'$ starting at the same node and arriving to the same node through different incoming arcs, contradicting \cref{lem:refinementSpines2}.
\end{proof}

\begin{lemma}
  \label{lem:refinementSpines4}
  The graph~$\spine'$ is indeed a spine on~$\decoratedGraph'$.
\end{lemma}

\begin{proof}
  First, $\spine'$ is a directed tree as it is obtained by contractions in~$\almostSpine'$ which is a directed tree by \cref{lem:refinementSpines3}.
...
\begin{proposition}
  ...
  The map~$\surjectionSpines$ is a filtered poset morphism from the spine poset~$\spinePoset[\decoratedGraph]$ to the spine poset~$\spinePoset[\decoratedGraph']$, meaning that
  \begin{itemize}
    \item if~$\spine_\circ \le \spine_\bullet$ in~$\spinePoset[\decoratedGraph]$, then~$\surjectionSpines(\spine_\circ) \le \surjectionSpines(\spine_\bullet)$ in~$\spinePoset[\decoratedGraph']$,
    \item the rank of~$\spine$ in~$\spinePoset[\decoratedGraph]$ is at most the rank of~$\surjectionSpines(\spine)$ in~$\spinePoset[\decoratedGraph']$.
  \end{itemize}
\end{proposition}
