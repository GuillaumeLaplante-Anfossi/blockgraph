...
We start by defining the ground sets on which our constructions hold.
We will use both maple trees and block graphs, which are closely related.
...
\begin{example}
  ...
  \end{enumerate}
  \vincent{picture of two spines on the block graph of \cref{fig:mapleBlock} (one maximal and one not).}
\end{example}
...
\begin{definition}
  \label{def:preorder}
  Recall that a \defn{preorder} on~$\vertexSet$ is a reflexive and transitive binary relation on~$\vertexSet$.
  For instance, an \defn{equivalence relation} is a symmetric preorder and a \defn{partial order} is an antisymmetric preorder.
  Any preorder~$R$ decomposes into an equivalence relation~$\equiv_R \eqdef \set{(u,v) \in R}{(v,u) \in R}$ together with a partial order~${R/\!\equiv_R}$ on the equivalence classes of~$\equiv_R$.
  For two preorders~$R, R'$ on~$\vertexSet$, we say that~$R$ \defn{extends}~$R'$ if $R \supseteq R'$ as subsets of~$\vertexSet \times \vertexSet$. %, \ie $u \, R' \, v$ implies $u \, R \, v$ for any~$u,v \in \vertexSet$.
\end{definition}

\begin{definition}
  \label{def:preorderSpine}
  Any spine~$\spine$ on~$\b{G}$ defines a preorder~$\preccurlyeq_{\spine}$ on~$\vertexSet$ by $u \preccurlyeq_{\spine} v$ if and only if there is a directed path in~$\spine$ from the node containing~$u$ ot the node containing~$v$.
...
\begin{remark}
  The lower (resp.~upper) ideal of the spine poset~$\spinePoset$ generated by a spine~$\spine$ are precisely the spines~$\spine'$ of~$\b{G}$ that extend (resp.~are extended by)~$\spine$ in the sense of \cref{def:preorderSpine}.
\end{remark}
...
\vincent{picture of two maximal spines connected by a flip.}
...
This is not strictly needed for the purposes of the next section, but it enlightens the picture.
...
%  Let~$\b{G}$ and~$\b{G}'$ be two decorated block graphs such that~$\b{G}$ refines~$\b{G'}$.
  The spine~$\surjectionSpines(\spine)$ is the unique minimal spine in the spine poset~$\spinePoset[\b{G}']$ extended by the spine~$\spine$ of~$\b{G}$.
\end{proposition}

\begin{proof}
  Observe first that~$\spine$ indeed extends~$\surjectionSpines(\spine)$ by \cref{lem:refinementSpines1}.
  Moreover, the spines of~$\b{G}'$ extended by~$\spine$ form an upper ideal of the spine poset~$\spinePoset[\b{G}']$.
  We thus need to show that this upper set is generated by~$\surjectionSpines(\spine)$.
  \vincent{todo}
\end{proof}

\begin{corollary}
  \label{coro:preimageRefinementSpines}
  For a spine~$\spine'$ on~$\b{G}'$, the preimage~$\surjectionSpines^{-1}(\spine')$ is the set of maximal spines on~$\b{G}$ that extend~$\spine'$ but no spine on~$\b{G}'$ strictly below~$\spine'$ in~$\spinePoset[\b{G}']$.
