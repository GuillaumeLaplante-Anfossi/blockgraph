...
\vincent{Say somewhere that the decoration of the blue leaves of~$\Maple$ (meaning the leaves of $\Maple$ minus its leaves) are irrelevant...}
...
%  Consider first a spine~$\spine$ on~$\decoratedGraph$ and its associated preorder~$\preccurlyeq_{\spine}$.
  ...
  By \cref{def:spine}, $Z_{k-1}$ and~$Z_{k+1}$ are separated by some~$z_k \in \Down[Z_k]$ (resp.~$\Up[Z_k]$) for each $i < k < j$ odd (resp.~even).
...
\begin{proof}
  ...
  We say that~$\pi$ is \defn{proper} if~$\Down[Z_k]$ (resp.~$\Up[Z_k]$) separates the union of the nodes of~$\pi$ before~$Z_k$ from the union of the nodes of~$\pi$ after~$Z_k$ for all~$i < k < j$ odd (resp.~even).
  ...
  Assume now that there is an undirected path~$\pi$ between~$X$ and~$Y$ in~$H$, and keep the labeling as before.
  ...
  If~$\pi'$ is incoming at~$X_k$, then denoting by~$Z'$ the first node of~$\pi'$ with two outgoing arcs, we have~$Z' \prec X_k \prec Z_k$ so that~$X_k$ and~$Z_k$ are not separated by~$\Up[Z']$ in~$\Graph$.
  Hence, $Z_k\pi$ is a proper undirected path joining~$Z_k$ to~$Y_k$, contradicting that~$Z_k$ is comparable to~$Y_k$.
  By symmetry, we obtain that $\pi'$ is outgoing at both~$X_k$ and~$Y_k$.
  We can thus replace~$X_k \rightarrow Z_k \leftarrow Y_k$ by the path~$\pi'$ in~$\pi$.
  With this substitution, we can thus assume that~$\Down[Z_k]$ separates~$X_k$ from~$Y_k$ in~$\Graph$.
  But~$Z_{k-1}$ and~$X_k$ (resp.~$Z_{k+1}$ and~$Y_k$) are not separated by~$\Down[Z_k]$ since~$Z_{k-1} \prec X_k \prec Z_k$ (resp.~since~$Z_k \succ Y_k \succ Z_{k+1}$).
  By symmetry, we can thus assume that~$\Down[Z_k]$ (resp.~$\Up[Z_k]$) separates~$Z_{k-1}$ from~$Z_{k+1}$ for all~$i < k < j$ odd (resp.~even), which implies that~$\pi$ is proper.
  
  We now conclude from this claim that~$H$ is a spine:
  \begin{itemize}
    \item $H$ is connected: Any two elements~$x,y$ are connected by a directed path in~$H$ if they are comparable in~$\preccurlyeq$, and by an undirected path otherwise.
    \item $H$ is acyclic: If it contains a cycle~$C$, then for an arbitrary arc~$(x,y) \in C$, we would have an undirected path between~$x$ and~$y$ whose endpoints are comparable.
    \item the nodes of~$H$ partition~$\vertexSet$: Indeed they are the classes of the equivalence relation~$\equiv_{\spine}$.
    \item each node of~$H$ satisfies the local condition of \cref{def:spine}: For any node~$X$ of~$H$, any two incoming arcs~$\alpha, \alpha'$ at~$Y$ and any nodes~$Y \in \sc[\alpha]$ and~$Y' \in \sc[\alpha']$, the unique (since~$H$ is acyclic) path from~$Y$ to~$Y'$ in~$H$ has two incoming arrows at~$X$. Since this path must be proper, $\Down[X]$ separates~$Y$ from~$Y'$. The argument is symmetric for outgoing arcs.
    \qedhere
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  This immediately follows from \cref{thm:alternativeDescriptionRefinementSpines}.
