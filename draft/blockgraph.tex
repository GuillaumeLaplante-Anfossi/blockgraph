...
\begin{definition}
  ...
  Fix~$\theta_v > 0$ for~$v \in \vertexSet$.
  The \defn{$\theta$-weights} are defined by~$\weight_{x,y} = \big( \sum_u \theta_u \big) \cdot \big( \sum_v \theta_v \big)$ where the first (resp.~second) sum ranges over all neighbors~$u$ of~$x$ (resp.~$v$ of~$y$) in~$\Graph$ which are not on the induced path joining~$x$ to~$y$.
\end{definition}
