...
\begin{proposition}
  ...
  If~$\decoratedGraph_\circ$ refines~$\decoratedGraph_\bullet$, then the map~$\surjectionSpines$ is 
  \begin{itemize}
    \item a surjection from the spines on~$\decoratedGraph_\circ$ to the spines on~$\decoratedGraph_\bullet$,
    \item a filtered map from the spine poset~$\spinePoset[\decoratedGraph_\circ]$ to the spine poset~$\spinePoset[\decoratedGraph_\bullet]$, meaning that the rank of~$\surjectionSpines(\spine_\circ)$ is at least the rank of~$\spine_\circ$ (it can have more, see \cref{fig:blossomingSpine}).
