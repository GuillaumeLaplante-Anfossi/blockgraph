...
\subsection{Blossoming spines and surjection maps}

In this section, we aim at describing a natural surjection from the ordered partitions (resp.~the permutations) of the vertex set of~$G$ to the spines (resp.~the maximal spines) on~$G$.
...
\begin{definition}
  ...
  A \defn{directed blossoming tree} is a directed tree with additional \defn{blossoms} (either incoming or outgoing) which are half-arcs.
\end{definition}

\begin{definition}
  \label{def:blossomingSpine}
  Let~$M$ be a maple tree and~$G$ be the block graph obtained by tapping~$M$.
  The \defn{blossoming spine} of a spine~$\spine$ on~$G$ is the directed blossoming tree obtained from~$\spine$ by
  \begin{itemize}
    \item labeling each arc~$\gamma$ of~$\spine$ by the red vertices of~$M$ which belong to both the connected component of~$\smash{M \ssm \up[{\so[\gamma]}]}$ containing~$\ta[\gamma]$ and the connected component of~$\smash{M \ssm \down[{\ta[\gamma]}]}$ containing~$\so[\gamma]$,
    \item adding at each node~$U$ of~$\spine$ an incoming (resp.~outgoing) blossom labeled by the the red vertices of~$C$ for each connected component~$C$ of~$M \ssm \down[U]$ (resp.~of~$M \ssm \up[U]$) such that there is no incoming arc~$\alpha$ (resp.~outgoing arc~$\beta$) of~$U$ with~$\so[\alpha]$ (resp.~$\ta[\beta]$) contained in~$C$.
  \end{itemize}
\end{definition}

\begin{proposition}
  \label{prop:blossomingSpine}
  Properties of blossoming spines.
\end{proposition}
