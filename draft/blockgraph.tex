...
\newcommandx{\HH}[1][1=\weight]{\mathbb{H}} % hyperplane
\newcommandx{\Hyp}[1][1=\weight]{\mathbf{H}} % halfspace
...
\begin{definition}
  ...
  In other words, it is the facet-ridge graph of the nested complex of~$G$.
\end{definition}
...
%\[
%\HH \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one}{\b{x}} = \weight_{V_G}}.
%\]
...
%More generally, we associate to an acyclic oriented graph~$H$ whose nodes partition~$V_G$ the cone~$\normalCone(H) \eqdef \set{\b{x} \in \HH}{x_u \le x_v \text{ for all } u \preccurlyeq_H v}$, where~$u \preccurlyeq_H v$ means that there is a directed path in~$H$ from the node containing~$u$ to the node containing~$v$ (in particular, ~$u =_H v$ if~$u$ and~$v$ belong to the same node of~$H$).
...
%\[
%  \normalCone(H) \eqdef \set{\b{x} \in \HH}{x_u \le x_v \text{ for all } u \preccurlyeq_H v}
%  \quad\text{and}\quad
...
%  \[
%    \normalCone(\spine) \eqdef \set{\b{x} \in \HH}{x_u \le x_v \text{ for all (possibly trivial) directed paths } u \to v \text{ in } \spine}
%  \]
...
%  The spine fan coarsens the braid fan and refines the graphical fan.
%\end{proposition}
%
%\begin{proof}
%  This immediately follows from \cref{subsec:surjectionMaps}.
...
\begin{proposition}
  ...
  If~$G$ refines~$G'$, the spine fan of~$G$ refines the spine fan of~$G'$.
  In particular, the spine fan coarsens the braid fan and refines the graphical fan.
\end{proposition}
...
Recall that a \defn{polytope} is the convex hull of a finite set of points, or equivalently a bounded intersection of a finite set of affine halfspaces.
The faces of a polytope are its intersections with its supporting affine hyperplanes.
The \defn{normal cone} of a face~$F$ of a polytope~$P$ is the cone of all linear functions whose maximum over~$P$ is attained precisely on~$F$.
The \defn{normal fan} of~$P$ is the fan formed by the normal cones of all the faces of~$P$.
...
\begin{example}
  ...
  \begin{itemize}
    ...
    \vincent{todo: be careful with weights...}
    \item the intersection of the affine hyperplane~$\HH \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one}{\b{x}} = \weight_{V_G}}$ with the affine halfspaces~$\Hyp_U \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one_U}{\b{x}} \ge \weight_U}$ for all subsets~$\varnothing \ne U \subsetneq V_G$.
  \end{itemize}
...
\begin{example}
  ...
  The normal fan of the graphical zonotope~$\Zono$ is the graphical fan~$\graphicalFan$.
  \vincent{todo}
\end{example}
...
\begin{theorem}
  ...
  For any weight matrix~$\weight$, the spine fan of~$G$ is the normal fan of the \defn{spine polytope} of~$G$ defined equivalently as
  \begin{enumerate}[(i)]
    \item the convex hull of the points~$\point \eqdef \sum_{\pi \in \Pi(\spine)} \weight_\pi \big( \one_{\peaks} - \one_{\valleys \ssm \boundary} \big)$ for all corank~$0$ spines~$\spine$~on~$G$,
    \item the intersection of the affine hyperplane~$\HH \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one}{\b{x}} = \weight_{V_G}}$ with the affine halfspaces~$\Hyp_B \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one_B}{\b{x}} \ge \weight_B}$ for all blocks of~$G$.
  \end{enumerate}
\end{theorem}
