...
\end{enumerate}
  ...
  Moreover, by \cref{def:nodeSplitting}, the arc~$\alpha$ is connected to the node~$U \cap B$ of~$\spine'$ if~$\so[\alpha] \subseteq B$, and to the node~$U \ssm B$ of~$\spine'$ if~$\so[\alpha] \negDisjoint B$.
...
\begin{definition}
  ...
  We denote by~$\equiv_G$ the equivalence relation on ordered partitions of~$V_G$ defined by~$\pi \equiv_G \pi'$ iff $\partitionToSpine(\pi) = \partitionToSpine(\pi')$.
  In other words, the equivalence classes of~$\equiv_G$ are the fibers of the map~$\partitionToSpine$.
  \end{definition}

\begin{remark}
  \label{rem:latticeCongruence}
  Note that~$\equiv$ has no reason to be a lattice congruence.
  \vincent{todo}
\end{remark}
...
\begin{definition}
  ...
  We say that~$G$ \defn{refines}~$G'$ (and that $G'$ \defn{coarsens} $G$) if they are decorations of the same underlying block graph and $\down[V_G] \subseteq \down[V_{G'}]$ while $\up[V_G] \subseteq \up[V_{G'}]$.
\end{definition}
...
%\[
%\Hyp \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one}{\b{x}} = \weight_{V_G}}.
%\]
...
\begin{example}
  ...
  Since this arrangement is not essential (all its hyperplanes contain the line generated by~$\one$), we consider its intersection with the hyperplane~${\Hyp[0] \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one}{\b{x}} = \weight_{V_G}}}$.
  ...
  In particular, the regions of~$\braidFan$ correspond to bijections~$V_G \to [|V_G|]$ and the rays of~$\braidFan$ correspond to proper subsets of~$V_G$ (neither empty nor all~$V_G$).
  \vincent{Not sure if we want surjections or ordered partitions here.}
%More generally, we associate to an acyclic oriented graph~$H$ whose nodes partition~$V_G$ the cone~$\normalCone(H) \eqdef \set{\b{x} \in \Hyp[0]}{x_u \le x_v \text{ for all } u \preccurlyeq_H v}$, where~$u \preccurlyeq_H v$ means that there is a directed path in~$H$ from the node containing~$u$ to the node containing~$v$ (in particular, ~$u =_H v$ if~$u$ and~$v$ belong to the same node of~$H$).
%Recall that a \defn{preposet} on~$V_G$ is a reflexive and transitive binary relation~$R \subsetea V_G \times V_G$.
...
%Let~$H$ be an acyclic oriented graph~$H$ whose nodes partition~$V_G$.
%For two vertices~$u,v \in V_G$, we write~$u \preccurlyeq_H v$ if there is a directed path in~$H$ from the node containing~$u$ to the node containing~$v$ (in particular, ~$u =_H v$ if~$u$ and~$v$ belong to the same node of~$H$).
...
\begin{example}
  ...
  \begin{itemize}
%    \item the convex hull of the points~$\sum_{u,v \in V_G} \delta_{\sigma(u) \le \sigma(v)} \weight_{uv} \b{e}_v$ for all bijections~$\sigma: V_G \to [|V_G|]$,
    \item the convex hull of the points~$\point[\sigma] \eqdef \sum_{p \in \monombinom{V_G}} \weight_p \b{e}_{\max_\sigma(p)}$ for all bijections~$\sigma: V_G \to [|V_G|]$,
    \item the intersection of the affine hyperplane~$\Hyp \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one}{\b{x}} = \weight_{V_G}}$ with the affine halfspaces~$\HS_U \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one_U}{\b{x}} \ge \weight_U}$ for all subsets~$\varnothing \ne U \subsetneq V_G$,
    \item the Minkowski sum~$\sum_{p \in \monombinom{V_G}} \weight_p \triangle_p$, where~$\triangle_p \eqdef \conv\set{\b{e}_v}{v \in p}$.
  \end{itemize}
  The braid fan~$\braidFan$ is the normal fan of the permutahedron~$\Perm$.
  Note that the barycenter of~$\Perm$ is the point~$\b{0}^\weight \eqdef \sum_{p \in \monombinom{V_G}} \frac{\weight_p}{|p|} \one_p$.
\end{example}
...
\begin{theorem}
  ...
  \begin{enumerate}[(i)]
    ...
    \item the intersection of the affine hyperplane~$\Hyp \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one}{\b{x}} = \weight_{V_G}}$ with the affine halfspaces~$\HS_B \eqdef \bigset{\b{x} \in \R^{V_G}}{\dotprod{\one_B}{\b{x}} \ge \weight_B}$ for all blocks of~$G$.
  \end{enumerate}
\end{theorem}

\vincent{Examples of vertices and facets from the pictures.}
...
\begin{proof}
  ...
  This shows that~$\dotprod{\one}{\point} = \sum_{\pi \in \Pi(\spine)} \weight_{\boundary} = \weight_{V_G}$ since any two vertices of~$V_G$ are connected by precisely one simple path in~$\spine$.
  ...
  We conclude that~$\dotprod{\one_{\so[\gamma]}}{\point} = \sum_{\pi \in \Pi(\spine), \; \boundary \subseteq \so[\gamma]} \weight_{\boundary} = \weight_{V_G}$ since any two vertices of~$\so[\gamma]$ are connected by precisely one simple path in~$\spine$.
\end{proof}
