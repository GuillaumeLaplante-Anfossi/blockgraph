...
In a maple tree, we label the red vertices by letters (or words) and the blue vertices by digits (or numbers).
Consequently, in a block graph, we label the vertices by digits (or numbers).
...
\begin{definition}
  ...
  For a subset~$V$ of vertices, we denote by~$\up[V]$ (resp.~$\down[V]$) the set of vertices~$\up$ (resp.~$\down$) and~$\updown$ of~$V$.
\end{definition}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Spine refinement poset}
...
\begin{definition}
  The \defn{spine refinement poset} is the poset on spines on~$G$ defined by~$\spine \leq_G \spine'$ if and only if any node of~$\spine'$ is contained in a node of~$\spine$. 
\end{definition}

In the remaining of this section, we charaterize the covering relations in the spine refinement poset in terms of two inverse operations on spines: edge contraction and vertex splitting.
...
\begin{corollary} 
 The spine refinement poset is graded by the number of nodes. 
...
\begin{lemma} 
  ...
  The spines~$\spine$ and $\spine'$ are the only spines covering $\spine''$ in the spine refinement poset.
\end{lemma}
